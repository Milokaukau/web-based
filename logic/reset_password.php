...
if ($token === ''){
    redirectWith('/pages/forgot_password.php', 'error', 'Invalid or missing reset link. Please try again.');
}

$token_row = getResetToken($token);

if (!$token_row){
    redirectWith('/pages/forgot_password.php', 'error', 'This reset link has expired or is invalid. Please request a new one.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if (empty($errors)){
        $member = getMemberById($token_row->member_id);
        if ($member && password_verify($new_pass, $member->password)) {
